...
async function fetchHourlyForecast(lat, lng) {
  ...
  try {
    ...
    const hourlyForecasts = forecastData.forecast.forecastday[0].hour.slice(0, 8); // Limit to 8 hours for mobile
...
async function fetchSuggestions() {
  ...
  try {
    ...
    suggestionList.innerHTML =
      '<li style="color: var(--danger-color); pointer-events: none;">Error fetching suggestions.</li>';
  }
}
...
async function fetchForecast(lat, lng) {
  ...
  try {
    ...
    const dailyForecasts = forecastData.list.filter((_, index) => index % 8 === 0).slice(0, 5); // Limit to 5 days for mobile
...
document
  ...
  .addEventListener("input", () => debounce(fetchSuggestions, 300));